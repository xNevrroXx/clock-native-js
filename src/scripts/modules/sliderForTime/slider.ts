import getCountPixels from "../techFunctions/getCountPixels";
...
function createSlider(containerElement: HTMLElement, maxValue: number, defaultActiveNumber = 0) {
    ...
    if (!wrapperSlides) throw new Error("Could not find wrapper slides");
    ...
    const activeElement: HTMLElement | null = wrapperSlides.querySelector('span.active');
    if (!activeElement) throw new Error("Could not find active element");
    ...
    function changeSlide(this: HTMLElement, event: Event) {
        /*
