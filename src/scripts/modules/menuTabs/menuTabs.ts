// own modules
// import initSlider from "../sliderForTime/initSlider";
...
function menuTabs(currentTab: TClockTasks/*todo some function that creates one of the clock types*/) {
    ...
    if (!tabsElement) throw new Error("Header with tabs not found");
    ...
    function handler(event: Event) {
        if(event.target instanceof HTMLElement) {
            if(event.target.tagName == "DIV" && event.target.textContent && event.target.textContent.toLowerCase() !== activeTab) {
